# -*- encoding: utf-8 -*-
# stub: pry-awesome_print 9.6.11 ruby lib

Gem::Specification.new do |s|
  s.name = "pry-awesome_print"
  s.version = "9.6.11"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib"]
  s.authors = ["Barry Allard"]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"]
  s.date = "2016-07-10"
  s.description = "Combine 'pry' with 'awesome_print' without hacking pryrc."
  s.email = "barry.allard@gmail.com"
  s.homepage = "https://github.com/steakknife/pry-awesome_print"
  s.licenses = ["MIT"]
  s.rubygems_version = "2.4.8"
  s.summary = "Pry + awesome_print."

  s.installed_by_version = "2.4.8" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<awesome_print>, ["< 999", ">= 1.1.0"])
      s.add_runtime_dependency(%q<pry>, ["< 999", ">= 0.9.0"])
    else
      s.add_dependency(%q<awesome_print>, ["< 999", ">= 1.1.0"])
      s.add_dependency(%q<pry>, ["< 999", ">= 0.9.0"])
    end
  else
    s.add_dependency(%q<awesome_print>, ["< 999", ">= 1.1.0"])
    s.add_dependency(%q<pry>, ["< 999", ">= 0.9.0"])
  end
end
